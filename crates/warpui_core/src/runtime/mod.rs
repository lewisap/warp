...
/// Dispatches a converted input [`Event`] into the shared core for `window_id`'s
/// view tree, returning whether it was handled.
///
/// Screen-independent on purpose: it takes the laid-out `size` rather than a
/// terminal, so the input path can dispatch without borrowing the
/// [`TuiScreen`]. It runs the GUI-parity keymap pass (the focused view's
/// responder chain) first, then walks the rendered element tree, finally
/// draining the app updates and typed actions queued during dispatch.
fn dispatch_event(
    ctx: &mut AppContext,
    window_id: WindowId,
    ...
    for action in event_ctx.take_typed_actions() {
        ...
        // ancestors in the neutral view hierarchy), so an action raised inside
        // an embedded child view bubbles to ancestor handlers.
        ...
        match event {
            ...
            event => {
                if let Some(warp_event) = crossterm_event_to_warp_event(event) {
                    let size = self.screen.size().unwrap_or_default();
                    let window_id = self.screen.window_id;
                    let root_view_id = self.screen.root_view_id();
                    let handled = app.update(|ctx| {
                        dispatch_event(ctx, window_id, root_view_id, size, &warp_event)
                    });
                    if handled {
                        self.dirty.set(true);
                    }
