...
impl TuiElement for TuiScrollable {
    fn layout(&mut self, constraint: TuiConstraint) -> TuiSize {
        ...
        let content_height = self.child.desired_height(width);
        // Lay the child out at its full natural height so children that cache
        // their arrangement (e.g. a centered child) paint content-sized; the
        // viewport clips during render.
        self.child.layout(TuiConstraint::new(
            TuiSize::new(width, 0),
            TuiSize::new(width, content_height),
        ));
    ...
    fn render(&self, area: TuiRect, buffer: &mut TuiBuffer) {
        ...
        self.child.render(content_rect, &mut content);
    ...
    fn dispatch_event(
        ...
        let content_height = self.child.desired_height(area.width);
