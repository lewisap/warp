...
use crate::elements::tui::{
    TuiBuffer, TuiBufferExt, TuiConstraint, TuiElement, TuiEventContext, TuiRect, TuiSize, TuiText,
};
...
fn mouse_wheel_inside_the_viewport_scrolls() {
    App::test((), |app| async move {
        app.read(|app_ctx| {
            ...
            scrollable.layout(TuiConstraint::tight(area.as_size()));
    ...
    App::test((), |app| async move {
        app.read(|app_ctx| {
            ...
            let mut dispatch = |name: &str| {
                scrollable.layout(TuiConstraint::tight(area.as_size()));
                let mut event_ctx = TuiEventContext::default();
                scrollable.dispatch_event(&key(name), area, &mut event_ctx, app_ctx)
            };
...
fn offset_persists_in_the_handle_across_rebuilt_elements() {
    let handle = TuiScrollHandle::new();
    {
        ...
        first.layout(TuiConstraint::tight(TuiSize::new(1, 3)));
        handle.set_offset(5);
    }
